    ...
    private void setupCameraParameters() {
        ...
        if( is_manual_iso ) {
            if( supports_exposure_time ) {
                ...
                // For high speed video, if exposure time is default, use a reasonable value
                if( is_video && video_high_speed && exposure_time_value == CameraController.EXPOSURE_TIME_DEFAULT ) {
                    // Use 1/240s as default for slow motion (assuming 240fps)
                    exposure_time_value = 1000000000L / 240;
                    if( MyDebug.LOG )
                        Log.d(TAG, "using default exposure time for high speed video: " + exposure_time_value);
                }
